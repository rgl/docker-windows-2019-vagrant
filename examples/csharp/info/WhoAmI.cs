...
public class WhoAmI
{
    ...
    {
        public Account(string sid, string domain, string name, string type)
        {
            Sid = sid;
            Domain = domain;
            Name = name;
    ...
    public class Whoami
    {
        public Whoami(AccountAndAttributes user, AccountAndAttributes[] groups, PrivilegeAndAttributes[] privileges)
        {
            User = user;
            Groups = groups;
        ...
        public AccountAndAttributes User { get; }
        public AccountAndAttributes[] Groups { get; }
        public PrivilegeAndAttributes[] Privileges { get; }
    }

    public class AccountAndAttributes
    {
        public AccountAndAttributes(Account account, string[] attributes)
        {
            Account = account;
    ...
    {
        public PrivilegeAndAttributes(string privilege, string[] attributes)
        {
            Privilege = privilege;
            Attributes = attributes;
        }
    ...
    private static Account GetAccount(IntPtr sid)
    {
        ...
        return new Account(
            SidToString(sid),
            domainStringBuilder.ToString(),
            nameStringBuilder.ToString(),
            nameUse.ToString()
        );
    }
        ...
        try
        {
            ...
            return groups.Select(
                    g => new AccountAndAttributes(
                        GetAccount(g.Sid),
                        GetGroupAttributes(g.Attributes)
                    )
                ).ToArray();
        }
        finally
        ...
        try
        {
            ...
            return privileges.Select(
                    p => new PrivilegeAndAttributes(
                        GetPrivilegeName(p.Luid),
                        GetPrivilegeAttributes(p.Attributes)
                    )
                ).ToArray();
        }
        finally
